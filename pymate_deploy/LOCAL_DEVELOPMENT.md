...
```bash
# 1. 예제 파일 복사
cp pymate_deploy/.env.local.example .env.local
...
```bash
# Docker 컨테이너 시작
docker compose -f pymate_deploy/docker-compose.local.yml --env-file .env.local up

# 또는 백그라운드 실행 (-d 옵션)
docker compose -f pymate_deploy/docker-compose.local.yml --env-file .env.local up -d
```
...
```bash
# 1. Docker 컨테이너 실행 중
docker compose -f pymate_deploy/docker-compose.local.yml up

# 2. VSCode 등에서 Django 코드 수정
# app/apps/chat/views.py 수정
...
# 컨테이너 중지
docker compose -f pymate_deploy/docker-compose.local.yml down

# 컨테이너 재시작
docker compose -f pymate_deploy/docker-compose.local.yml restart

# 컨테이너 상태 확인
docker compose -f pymate_deploy/docker-compose.local.yml ps
```
...
```bash
# 전체 로그
docker compose -f pymate_deploy/docker-compose.local.yml logs

# Django 로그만
docker compose -f pymate_deploy/docker-compose.local.yml logs django

# Qdrant 로그만
docker compose -f pymate_deploy/docker-compose.local.yml logs qdrant

# 실시간 로그 모니터링 (Ctrl+C로 종료)
docker compose -f pymate_deploy/docker-compose.local.yml logs -f
```
...
python app/manage.py migrate          # DB 마이그레이션
python app/manage.py createsuperuser  # 관리자 계정 생성
python app/manage.py shell            # Django shell
```
...
docker compose -f pymate_deploy/docker-compose.local.yml down -v
...
```bash
# 1. 볼륨 마운트 확인
docker compose -f pymate_deploy/docker-compose.local.yml config

# 2. 컨테이너 재시작
docker compose -f pymate_deploy/docker-compose.local.yml restart django
...
2. **작업 흐름**
   ```bash
   # 1. Docker 컨테이너 시작
   docker compose -f pymate_deploy/docker-compose.local.yml up -d
