...
|**안혜빈**|RAG 기반 챗봇의 성능 개선을 맡아 진행했다. 사용자와의 대화 기억이나 code 제공 답변과 같은 방식으로 검색 결과의 적합도를 높이고 사용자 질문에 더 알맞는 응답을 생성하는 구조를 만들고자 했다. 
프로젝트를 진행하면서 RAG 성능 개선이 단순한 파라미터 조정이나 모델 변경만으로 해결하기 어렵고, 데이터 전처리, 청킹, 검색-응답 생성 프롬프트가 모두 연결되어있는 것이라는 것을 체감하였다. 
여러 시도를 해보고 다양하게 학습해보았지만, 제한된 시간과 경험 부족으로 스스로에게 아쉬운 마음이 들지만 함께 너무 잘 진행해준 팀원들에게 크게 감사하다!
이번 프로젝트를 통해 RAG 구조, 웹 서비스 배포 구조에 대해 더 깊이 이해해야 할 필요성을 충분히 느꼈고, 다음 프로젝트에서 더 성장해야겠다!|
