...
|**신지용**|3차 프로젝트에서 RAG 설계, 데이터 전처리 및 검색 에이전트 개발을 담당했던 경험을 바탕으로, 이번에는 이를 실제 웹 서비스로 확장하는 데 주력했습니다. AWS EC2 인프라 위에 Docker와 Docker Compose를 활용해 Django, Nginx, Qdrant를 컨테이너화하여 배포 환경을 일원화했으며, Gunicorn과 Nginx 역방향 프록시를 구성해 서비스 안정성을 확보했습니다. 특히 쉘 스크립트로 EC2 초기 설정과 배포 과정을 자동화하며 DevOps의 효율성을 경험했습니다. AWS 네트워크 설정 등 생소한 인프라 이슈로 잦은 시행착오를 겪었지만, 이를 통해 실제 서비스 아키텍처를 깊이 이해하는 계기가 되었습니다. 믿고 맡겨준 팀원들 덕분에 인프라 구축이라는 새로운 도전을 완수할 수 있었습니다. 모두 고생 많으셨습니다!|
...
|**안혜빈**|RAG 기반 챗봇의 성능 개선을 맡아 진행했다. 사용자와의 대화 기억이나 code 제공 답변과 같은 방식으로 검색 결과의 적합도를 높이고 사용자 질문에 더 알맞는 응답을 생성하는 구조를 만들고자 했다. 프로젝트를 진행하면서 RAG 성능 개선이 단순한 파라미터 조정이나 모델 변경만으로 해결하기 어렵고, 데이터 전처리, 청킹, 검색-응답 생성 프롬프트가 모두 연결되어있는 것이라는 것을 체감하였다. 여러 시도를 해보고 다양하게 학습해보았지만, 제한된 시간과 경험 부족으로 스스로에게 아쉬운 마음이 들지만 함께 너무 잘 진행해준 팀원들에게 크게 감사하다! 이번 프로젝트를 통해 RAG 구조, 웹 서비스 배포 구조에 대해 더 깊이 이해해야 할 필요성을 충분히 느꼈고, 다음 프로젝트에서 더 성장해야겠다!|
